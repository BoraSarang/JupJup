...
class NewsRepository(private val db: MacDatabase) {
    ...
    suspend fun list(filter: NewsFilter): PagedNews {
        ...
        val rows = db.newsArticleDao().listFiltered(filter.main, filter.sub, sourceId, q, size, offset)
        val total = db.newsArticleDao().countFiltered(filter.main, filter.sub, sourceId, q)
