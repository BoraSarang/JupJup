...
import com.borasarang.macjupjup.ui.main.MainActivity
import org.junit.Rule
import org.junit.Test
import org.junit.runner.RunWith

/** 스모크: 4개 탭 전환 + 홈/설정 핵심 요소 노출 */
...
class MainActivityTest {
    ...
    @Test
    fun bottomNav_switchesThroughAllTabs() {
        ActivityScenario.launch(MainActivity::class.java).use {
            onView(withId(R.id.nav_source)).perform(click())
            onView(withId(R.id.source_recycler)).check(matches(isDisplayed()))
            onView(withId(R.id.nav_notif)).perform(click())
            onView(withId(R.id.notif_recycler)).check(matches(isDisplayed()))
            onView(withId(R.id.nav_settings)).perform(click())
            onView(withId(R.id.battery_status)).check(matches(isDisplayed()))
            onView(withId(R.id.et_token)).check(matches(isDisplayed()))
            onView(withId(R.id.switch_translate_ko)).check(matches(isDisplayed()))
