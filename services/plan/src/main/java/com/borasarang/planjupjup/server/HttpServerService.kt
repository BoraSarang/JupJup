...
class HttpServerService : Service() {
    ...
    internal fun app(): PlanJupJupRuntime = PlanJupJupRuntime
    ...
    override fun onCreate() {
        ...
        startInForeground(getString(R.string.plan_notif_server_starting))
        scope.launch {
            try {
    ...
    private fun startServer(port: Int) {
        server = embeddedServer(CIO, port = port, host = "0.0.0.0") {
            ...
            com.borasarang.common.server.AdminAuth.install(this) { app().preferences.getAdminToken() }
            install(StatusPages) {
            ...
            routing {
                planAssetRoutes(this)
                planItemRoutes(this)
                planCollectRoutes(this)
                planStatsRoutes(this)
                planNotifRoutes(this)
                planSettingsRoutes(this)
            }
        }.start(wait = false)
    }
    ...
    private fun runningText(port: Int): String {
        ...
        return getString(R.string.plan_notif_server_running) + " http://$ip:$port"
    }

    private fun buildNotification(text: String): Notification {
        return Notification.Builder(this, Constants.CHANNEL_ID_SERVER)
            .setContentTitle(getString(R.string.plan_app_name_full))
            .setContentText(text)
        ...
        try {
            ...
            nm.createNotificationChannel(
                NotificationChannel(
                    ...
                    getString(R.string.plan_notif_channel_server),
