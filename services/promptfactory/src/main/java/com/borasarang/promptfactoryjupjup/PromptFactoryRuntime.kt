package com.borasarang.promptfactoryjupjup
...
import com.borasarang.promptfactoryjupjup.worker.PromptFactoryScheduler
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.SupervisorJob
import kotlinx.coroutines.launch

object PromptFactoryRuntime {
    ...
    lateinit var database: PromptFactoryDatabase
        private set
    ...
    lateinit var providerKeys: ProviderKeyStore
        private set
    lateinit var scheduler: PromptFactoryScheduler
        private set
    ...
    fun initialize(context: Context) {
        ...
        DebugLogger.i("앱", "프롬프트팩토리 시작")

        database = try {
            PromptFactoryDatabase.getInstance(appContext)
        } catch (e: Exception) {
            DebugLogger.e("앱", Constants.ERR_DB_INIT_FAILED, "DB 열기 실패, 백업 후 재생성: ${e.message}", e)
            backupDatabaseFile()
            PromptFactoryDatabase.resetInstance()
            PromptFactoryDatabase.getInstanceFallback(appContext)
        }
    ...
    private fun backupDatabaseFile() {
        try {
            val src = appContext.getDatabasePath(PromptFactoryDatabase.DB_NAME)
            if (!src.exists()) return
            val dir = java.io.File(appContext.filesDir, "db-backup").apply { mkdirs() }
            val dst = java.io.File(dir, "${PromptFactoryDatabase.DB_NAME}.${System.currentTimeMillis()}.bak")
