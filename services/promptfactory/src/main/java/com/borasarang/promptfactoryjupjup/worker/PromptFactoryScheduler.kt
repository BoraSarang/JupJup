...
class PromptFactoryScheduler(private val context: Context) {
    ...
    suspend fun scheduleOne(prompt: Prompt) {
        ...
        when (prompt.scheduleType) {
            "daily" -> {
                val req = PeriodicWorkRequestBuilder<PromptFactoryWorker>(24, TimeUnit.HOURS)
            ...
            "once" -> {
                val req = OneTimeWorkRequestBuilder<PromptFactoryWorker>()
                    .setConstraints(constraints())
                    .setInitialDelay(delayMs, TimeUnit.MILLISECONDS)
                    .setInputData(data)
                    .addTag(TAG_PROMPT)
    ...
    private suspend fun getAllPrompts(): List<Prompt> {
        return runCatching {
            com.borasarang.promptfactoryjupjup.PromptFactoryRuntime.promptRepository.getEnabled()
        }.getOrDefault(emptyList())
    }

    private fun uniqueName(promptId: Long) = "pf_prompt_$promptId"
    ...
    companion object {
        private const val TAG_PROMPT = "promptfactory_prompt"
