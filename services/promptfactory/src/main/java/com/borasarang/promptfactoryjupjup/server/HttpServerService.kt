...
class HttpServerService : Service() {
    ...
    internal fun app(): PromptFactoryRuntime = PromptFactoryRuntime

    override fun onCreate() {
        super.onCreate()
        DebugLogger.i("서버", "프롬프트팩토리 서버 생성")
        createChannel()
        startInForeground(getString(R.string.pf_notif_server_starting))
        scope.launch {
            try {
    ...
    private fun runningText(port: Int): String {
        ...
        return getString(R.string.pf_notif_server_running) + " http://$ip:$port"
    }

    private fun buildNotification(text: String): Notification {
        return Notification.Builder(this, CHANNEL_ID)
            .setContentTitle(getString(R.string.pf_app_name_full))
            .setContentText(text)
    ...
    private fun createChannel() {
        try {
            ...
            nm.createNotificationChannel(
                NotificationChannel(
                    CHANNEL_ID,
                    getString(R.string.pf_notif_channel_server),
    ...
    companion object {
        const val ACTION_RESTART = "com.borasarang.promptfactoryjupjup.RESTART_SERVER"
        const val NOTIFICATION_ID = 3100
        const val CHANNEL_ID = "jupjup_pf_server"
