...
- `data/preferences/ModelEnabledStore.kt` 신규 (DataStore `pj_models`,
...
- `PromptJournalRuntime.initialize`: `attachStore` + `appScope`에서 `restoreEnabled()`.
- 호출자: `PjRoutes` 3곳 suspend 컨텍스트 그대로 (서명만 맞춤).
