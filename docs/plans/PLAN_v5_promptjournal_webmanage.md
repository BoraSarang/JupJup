# PLAN_v5 — 프롬프트 저널 다중 프롬프트 재설계

> 플랫폼: Android · v1.11.0 · 2026-09-18
> 부모: PLAN_v4_promptjournal.md (v1.10.0 단일 프롬프트 구조 → 다중 프롬프트 전환)
...
| API 키 저장 | DataStore `pj_api_keys`, 공급자별 `api_key_$provider` (웹에서 등록) |
...
| `PjSettings` (축소) | port/autoStart만 (단일 프롬프트 필드 전부 제거) |
...
- WorkRequest 고유명: `pj_prompt_{id}` (`ExistingPeriodicWorkPolicy.REPLACE`)
- `rescheduleAll()`: DB 활성 프롬프트 전체 재예약 (시작 시 / 활성 변경 시)
- `scheduleOne(prompt)`: 등록/수정/토글 시 단건 싱크
- `triggerImmediate(promptId)`: 즉시 실행 (고유명 `pj_immediate_{id}`, KEEP)
...
- PjServiceAdapter: 활성 프롬프트 기준 표시, `triggerImmediate(first.id)`
...
| 단계 | 작업 | 상태 |
|------|------|------|
| PJ5-1 | DB v2 + Prompt/DAO + ProviderKeyStore + 저장소 + ModelCatalog | ✅ |
| PJ5-2 | 스케줄러·워커 프롬프트id별 + 주입 | ✅ |
| PJ5-3 | 라우트 v2 + Runtime 시드 | ✅ |
| PJ5-4 | 웹 v2 + 앱 축소 + 어댑터 | ✅ |
| PJ5-5 | 빌드·테스트·lint·실기 검증 | ✅ |
| PJ5-6 | 문서 갱신 + 1.11.0 bump | 진행 중 |
