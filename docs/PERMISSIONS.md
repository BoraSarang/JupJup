...
> 통합 앱이 사용하는 권한과 근거. (mac/plan/pj/community 네 서비스 동일 권한 집합)
...
- 관리 인증 (R44/R48): `/api` 쓰기는 인증 필수. 앱 정보에서 관리자 ID/PW 1회 등록 → 4서비스 공통 적용, 미등록 시 서비스별 토큰 폴백. 토큰·PW는 DataStore 평문(루팅 노출 감수, 커밋·로그 금지), 조회는 루프백 페어링으로만.