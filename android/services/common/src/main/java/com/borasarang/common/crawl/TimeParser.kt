...
/** 게시 시각 관대 파서 (community 슈퍼셋 — 2자리 연도 포함). 실패 시 null */
object TimeParser {
    ...
    private val wsSplitRe = "\\s+".toRegex()
    private val dateSepRe = "[./-]".toRegex()
    ...
    private fun parseDateTime(text: String): Long? {
        return try {
            val parts = text.split(wsSplitRe)
            val d = parts[0].split(dateSepRe).map { it.toInt() }
    ...
    private fun parseDate(text: String): Long? {
        return try {
            val d = text.split(dateSepRe).map { it.toInt() }
            val cal = Calendar.getInstance()
